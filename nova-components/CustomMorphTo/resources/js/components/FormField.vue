<template>
  <div>
    <default-field :field="field" :show-errors="false" :field-name="fieldName" v-if="!isReadonly">
        <template slot="field">
            <!-- Morph type selector -->
            <div class="w-full mt-2">
                <a class="inline-block cursor-pointer mr-4 animate-text-color select-none mini-tab"
                    v-for="option in field.morphToTypes"
                    :class="{ 'active': resourceType == option.value }"
                    @click="changeResourceType(option)">
      ...
      <template slot="field">
        <div class="flex items-center mb-3">
          ...
          <div v-if="isReadonly" class="w-full pt-2">
            {{ selectedResource ? selectedResource.display : '—' }}
          </div>
          <multiselect v-else class="text-80 pb-2 leading-tight" track-by="value" label="display" 
            v-model="selectedResource" :options="availableResources" :dusk="field.attribute"
            @change="selectResourceFromMultiselectControl">
          </multiselect>
          <!--
          <select-control
...
import storage from '~~nova~~/storage/MorphToFieldStorage'
import {
  FormField,
  PerformsSearches,
  TogglesTrashed,
...
export default {
  ...
  methods: {
    ...
    changeResourceType: function (option) {
        if (this.resourceType == option.value) {
          return;
        }
        this.refreshResourcesForTypeChange({
            target: {
                value: option.value,
            }
        });
    ...
    selectResourceFromMultiselectControl(resource) {
