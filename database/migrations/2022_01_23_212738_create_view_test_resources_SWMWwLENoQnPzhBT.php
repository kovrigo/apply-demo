...
class CreateViewTestResourcesSWMWwLENoQnPzhBT extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {      
        ...
        DB::statement($this->createView('test_resources', true));
        if (true) {
            // Workflow
            DB::statement($this->deleteView('test_resource_workflows'));
            DB::statement($this->createView('test_resource_workflows', true));
            // Workflow - UserProfile
            DB::statement($this->deleteView('test_resource_workflow_user_profile'));
            DB::statement($this->createView('test_resource_workflow_user_profile', false));
            // State
            DB::statement($this->deleteView('test_resource_states'));
            DB::statement($this->createView('test_resource_states', true));
            // Log
            DB::statement($this->deleteView('test_resource_logs'));
            DB::statement($this->createView('test_resource_logs', true));
        }
    }
